        ...
        rows.append(
            [
                InlineKeyboardButton(
                    f"{prefix}{title}", callback_data=f"file_mode:set:{value}"
                )
            ]
        )
...
def language_keyboard(
    registry: LanguageRegistry, callback_prefix: str, include_auto: bool = True
    ...
    for item in top:
        rows.append(
            [
                InlineKeyboardButton(
                    registry.compact_label(item["code"]),
                    callback_data=f"{callback_prefix}:{item['code']}",
                )
            ]
        )
    rows.append(
        [InlineKeyboardButton("Другой...", callback_data=f"{callback_prefix}:other")]
    )
...
def live_popular_pairs_keyboard(
    registry: LanguageRegistry, mode: str
) -> InlineKeyboardMarkup:
    rows = []
    for pair in registry.popular_pairs:
        ...
        rows.append(
            [InlineKeyboardButton(title, callback_data=f"live_pair:set:{a}:{b}")]
        )
    rows.append(
        [InlineKeyboardButton("Другие языки", callback_data="live_pair:custom:start")]
    )
...
def file_result_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "Кратко", callback_data="file_result:show:summary"
                ),
                InlineKeyboardButton(
                    "Построчно", callback_data="file_result:show:bilingual"
                ),
            ],
            [
                InlineKeyboardButton(
                    "Стандарт", callback_data="file_result:show:standard"
                )
            ],
            [
                InlineKeyboardButton(
                    "🌍 Другой язык", callback_data="file_result:open:lang"
                ),
                InlineKeyboardButton("⚙️ Режим файла", callback_data="file:open:mode"),
            ],
            [
                InlineKeyboardButton(
                    "🔁 Повторить", callback_data="file_result:rerun:last"
                )
            ],
        ]
    )
...
def live_force_language_keyboard(
    a: str, b: str, registry: LanguageRegistry
) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    f"Считать как {registry.compact_label(a)}",
                    callback_data=f"live:force_source:{a}",
                )
            ],
            [
                InlineKeyboardButton(
                    f"Считать как {registry.compact_label(b)}",
                    callback_data=f"live:force_source:{b}",
                )
            ],
